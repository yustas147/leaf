...
class WooImporter(Importer):
    ...
    def run(self, woo_record=False, force=False):
#    def run(self, woo_id, force=False):
        """ Run the synchronization

        :param woo_id: identifier of the record on WooCommerce
        :param woo_record: raw data of the record on WooCommerce
        """
        if woo_record:
            self.woo_record = woo_record
            self.woo_id = woo_record['global_id']
#        self.woo_id = woo_id
   #     try:
...
#    def _import_record(self, record_id):
        ...
        import_record_straight(self.session,
...
#@related_action(action=link)
def import_record_straight(session, model_name, backend_id, woo_record, force=False):
#def import_record_straight(session, model_name, backend_id, woo_id, force=False):
    """ Import a record from Woo """
    env = get_environment(session, model_name, backend_id)
    importer = env.get_connector_unit(WooImporter)
    importer.run(woo_record = woo_record, force=force)
#    importer.run(woo_id, force=force)
